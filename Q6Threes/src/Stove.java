...
class Stove extends Thread {
    ...
    public void prepare() {
        ...
        synchronized (A) {
            /* Q4 wait in case that A is empty → suspend thread until object is invoked by other thread by notify()
             *  (relevant because intermediate stock will be empty initially and then repeatedly */
            while (A.isEmpty()) {
                try {
                    A.wait();
            ...
            A.get();
            System.out.println(Thread.currentThread().getName() + " (get): stock " + A.getName() + " contains " + A.getNbFood() + " food.");
        }
        ...
        synchronized (B) {
            B.put();
            /* Q4 invoke thread for this object again (might be suspended if it was empty) */
            B.notify();
            System.out.println(Thread.currentThread().getName() + " (put): stock " + B.getName() + " contains " + B.getNbFood() + " food.");
        }
    }
